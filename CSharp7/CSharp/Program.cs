...
in company.Customers
                    ...
                    using (SqlCommand cmd = new SqlCommand("SELECT * FROM Student", connection))
                    {
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                            ...
                            while (reader.Read())
                            {
                                ...
                                Console.WriteLine($"StudentID: {StudentID}, FirstName: {FirstName}, MiddleName: {MiddleName}, LastName: {LastName}, Gender: {Gender}, EnrollmentDate: {EnrollmentDate}");
                            }
                        }
                    }
                }
    ...
    public class UniversityDbContext : DbContext
    {
        ...
        public virtual DbQuery<StudentEnrollment> StudentEnrollments { get; set; }
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder
                .Query<StudentEnrollment>().ToView("StudentEnrollment_View")
    ...
    public class EntityFrameworkTestSuite : TestSuite, ITestSuite
    {
        ...
        private void EntityFramework5()
        {
            ...
            using (UniversityDbContext ctx = new UniversityDbContext())
            {
                List<StudentEnrollment> StudentEnrollments = ctx.StudentEnrollments.ToList();
                foreach (StudentEnrollment studentEnrollment in StudentEnrollments)
                {
                    Console.WriteLine($"First Name: {studentEnrollment.FirstName}, Middle Name: {studentEnrollment.MiddleName}, Last Name: {studentEnrollment.LastName}, Title={studentEnrollment.Title}");
                }
            }
        }

        private void EntityFramework6()
        {
            WriteMethodName(MethodBase.GetCurrentMethod().Name);
            using (UniversityDbContext ctx = new UniversityDbContext())
            {
                string commandText = "EXEC [dbo].[usp_GetAllStudents]";
                List<Student> Students = ctx.Students.FromSql(commandText).ToList();
                foreach (Student student in Students)
                {
                    Console.WriteLine($"First Name: {student.FirstName}, Middle Name: {student.MiddleName}, Last Name: {student.LastName}, Gender={student.Gender}");
                }
            }
        }
